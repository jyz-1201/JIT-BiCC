...
[Python](https://www.python.org/downloads/) and [Anaconda](https://docs.anaconda.com/anaconda/install/) installations are required to set up the environment. The necessary Python packages are stored in ```./requirements.yml```, which can be automatically installed using anaconda. We recommand setting up the environment on a Linux machine with nvidia GPU and [CUDA 11.1](https://developer.nvidia.com/cuda-11.1.0-download-archive).
